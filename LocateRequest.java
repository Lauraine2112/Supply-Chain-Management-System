...
public class LocateRequest {
	...
	public LocateRequest(String dburl, String username, String password, String furniture, String type, int amount) {
		...
		setFurniture(furniture.toLowerCase());
	...
	/**
	 * Setter method for private data member amount.
	 * Checks if input amount is valid - if amount is smaller or equal to zero, not valid
	 * @param amount the amount of furniture desired by the user
	 */
	 ...
	 * @return boolean 
	 */
	public static boolean getOrderIsPossible() { // new
	 ...
	 * @return void
	 */
	public void setOrderIsPossible(boolean orderIsPossible) { // new
	...
	public void getRequest() {
		...
		try {
			...
			//counts how many elements there are in table 'furniture' with the matching 'type'
			...
			while (rs.next()) {
				...
				colNum = rs.getInt(1); //keeps track of the last element that belongs in specific row of type 'furniture'
			}
		...
		if (this.hasExtra > 0) { // amount must be greater than one to use an extra
			...
			String[] newOrdered = new String[orderedIDs.length];
			System.arraycopy(orderedIDs, 0, newOrdered, 0, orderedIDs.length);
...
//		orderedIDs = null;
	...
	/**
	 * method addID creates a new array and copies all the IDs in unique to ordredIDs
	 * combines the old values of orderdIDs with the new ones contained in unique
	...
	public void addID(String[] unique) {
		...
		if (orderedIDs == null) {
			orderedIDs = new String[unique.length];
			...
			orderedIDs = new String[newIdLength];
	...
	public void removeFound() {
		...
		//copy values of of tempArr to foundRequest
		...
		for (String[] row : tempArr) { //goes through row  
			...
			//copies values of foundRequest to row
	...
	/**
	 * method checkRowToCopy checks if a row needs to be copied or not
	 * if an element is contained in bestCombination, returns false
	 * if an element is not containted, returns true
	 ...
	 * for a specific furniture type and category
	 * 
	 * @param j the integer that keeps track of the recursion. Always starts at 0
	 * @param row the number of rows in array foundRequest
	 * @param col the number of elements in a specific row (arms, legs... etc)
	 * @param first the double array of all elements for a specific furniture type and category
...
//		System.out.println(Arrays.toString(this.bestCombination));
	...
	public void addExtraPieces(String[] temp) {
		...
		this.foundRequest = new String[this.foundRequest.length + 1][this.foundRequest[0].length];
		...
		System.arraycopy(temp, 0, this.foundRequest[i], 0, temp.length);
	}
